...
fn preview_opencode_unknown_classification_fixture(
    ...
    let response = host.handle(ServiceRequest {
        ...
        params: json!({
            "agent": "opencode",
            "authorized_roots": [storage_root.to_string_lossy()],
            "auto_discover": false,
            "limit": 10,
...
fn preview_local_sessions_for_agent(host: &ServiceHost, agent: &str) -> Value {
    let mut params = json!({"agent": agent, "limit": 10, "max_excerpt_chars": 4_000});
    if agent == "opencode" {
        params["authorized_roots"] = json!([host
            .adapter_ctx
            .user_home
            .join(".local/share/opencode/storage")
            .to_string_lossy()]);
        params["auto_discover"] = json!(false);
    }
    let response = host.handle(ServiceRequest {
...
include!("local_session_preview_fixture.rs");
