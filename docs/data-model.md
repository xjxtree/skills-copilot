...
- `prompt-runs.json` stores redacted execution metadata for Agent Copilot's own
  ...
  provider draft output; their user-facing history keeps the complete redacted
  request and complete untrusted provider response only in native memory for
  the current app session. It is paged and date-filtered in the UI but has no
  fixed record-count eviction. Service startup nulls those fields in legacy
  Task Preflight rows while preserving their metadata and unrelated prompt
  runs.
...
- Once `(agent, scope, path)` identifies an existing skill instance, refreshes
  update that row without rewriting its `id`. This preserves catalog history
  and compatible app-local projections when a legacy display path later
  canonicalizes to the same stored path.
