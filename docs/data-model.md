...
- New model/task-match writes are disabled. A legacy
  `model-task-matches.json` is never treated as current product state and is
  exposed only as a content-free cleanup source through the explicit
  privacy-cleanup lifecycle.
- `prompt-runs.json` stores only bounded Agent Copilot action/provider metadata.
  Compatibility task and draft fields remain `null`, copy-only state is
  normalized to `false`, and raw prompt/response persistence flags remain
  false. Legacy task/draft bodies are suppressed on read and can be removed by
  the explicit privacy-cleanup lifecycle.
- `task-preflight-history.json` is legacy private content, not a current
  persistence model. Startup inspects it read-only; only an explicitly
  confirmed privacy cleanup can remove the bound leaf.
