...
| opencode | XDG/`OPENCODE_CONFIG_DIR` native roots; absolute `OPENCODE_CONFIG`; inline `OPENCODE_CONFIG_CONTENT`; selected-project ancestor native/official compatibility roots; local `skills.paths`. OpenCode may also expose runtime-owned built-in skills without a local source path, so the process-free product projection remains source-limited after a complete file scan. `opencode debug skill` may be used only as temporary acceptance evidence, never as an app source. | XDG data `opencode/opencode.db`; selected message detail uses a bounded fixed raw-row snapshot | `skills.urls`, remote organization config, disabled external/Claude compatibility sources, runtime inventory commands, caches, and legacy JSON sidecar stores when the SQLite DB exists |
