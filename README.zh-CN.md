...
- **全局搜索：** 从顶部工具栏搜索，并直接跳转到匹配的技能、会话或高级配置证据。
- **就绪度：** 确定性的本地证据用于报告环境覆盖；内联任务就绪度可匹配有效技能，
  未提供任务时不会声称某个 Agent 对所有工作都已就绪。
- **配置与证据：** 查看受支持 Agent 的配置快照、当前文件与诊断信息，预览回滚
  差异，并从“设置 › 高级”执行 App 支持的受保护配置操作。
- **技能包管理：** 通过“技能”中的集成操作和本地 `npx skills` 管理器搜索、预览、
  安装、更新、移除和创建本地技能包。
...
4. 选择项目并使用“项目概览”“技能”和“会话”；配置与恢复位于“设置 › 高级”。
